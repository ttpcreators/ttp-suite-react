...
export const RC_FIELDS: RcField[] = [
  ...
  { key: "collabAutonomeRegime", label: "Collaborations initiées par le Talent (Art. 7)", type: "select", default: "viaAgence", group: "Options", options: [{ value: "viaAgence", label: "Doivent passer par l'Agence" }, { value: "informer", label: "Gérées par le Talent, mais information de l'Agence" }, { value: "libre", label: "Gérées librement par le Talent, sans obligation d'informer" }], help: "Si le Talent démarche lui-même une marque / relance un ancien client : doit-il informer l'Agence, ou peut-il gérer entièrement de son côté ?" },
...
export const RC_VARIANTS: Variant[] = [
  {
    ...
    defaults: { variante: "exclusif", contratTest: "true", modeDuree: "determinee", dureeMois: "2", taciteReconduction: "false", preavisJours: "0", essaiActif: "true", essaiJours: "15", essaiParEmail: "false", mandatEncaissement: "true", reversJours: "7", contestationVirementJours: "5", gestionMail: "false", visibiliteEmail: "true", visibiliteMiseEnDemeureJours: "7", reportingMensuel: "true", modeCommission: "parOrigine", regimeFiscalComm: "HT", commApportee: "30", commInitiee: "15", commAutonome: "0", seuil: "100", avantagesNatureInclus: "true", giftingSeuilStrict: "false", exclusivite: "true", collabAutonomeRegime: "viaAgence", clausePenaleActive: "true", clausePenaleMois: "2", retardPaiementJours: "30", imageApresTermeMois: "12", confidentialiteApresAns: "3", approcheDirecteHeures: "48", charteEthique: "true", mediationOrg: "cciLyon", mediationDelaiJours: "30", talentStatutTva: "franchise" },
  },
  {
    key: "nonexclusif",
...
function ph(key: string, c: Record<string, string>): string {
  switch (key) {
    ...
    case "art7Collab": {
      const regime = c.collabAutonomeRegime || (c.variante === "exclusif" ? "viaAgence" : "informer");
      if (regime === "libre")
        return "Collaborations initiées par le Talent — Le Talent demeure libre d'initier, de négocier et de gérer intégralement, par ses propres moyens (démarchage direct d'une marque, relance d'anciens partenaires, contacts personnels), les collaborations qu'il obtient de son propre chef. Ces collaborations, menées de bout en bout par le Talent, sont hors du périmètre du présent mandat : elles n'ouvrent droit à aucune commission au profit de l'Agence, et le Talent n'est pas tenu d'en informer cette dernière.";
      if (regime === "informer")
        return "Collaborations initiées par le Talent — Le Talent peut initier et gérer lui-même des collaborations qu'il obtient par ses propres moyens (démarchage direct d'une marque, relance d'anciens partenaires). Il s'engage toutefois à en informer l'Agence, à des fins de transparence, de suivi et de cohérence de son image ; la commission éventuellement applicable est celle prévue à l'article 4.";
      return "Collaborations initiées par le Talent — Toute opportunité de collaboration commerciale que le Talent viendrait à initier lui-même (démarchage direct d'une marque, relance d'anciens partenaires) est portée à la connaissance de l'Agence, qui en assure la gestion, la négociation et la conclusion. Le Talent s'interdit de conclure directement une collaboration commerciale sans l'intervention préalable de l'Agence.";
    }
