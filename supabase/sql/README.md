...
| **Rôles agence fondateur/membre** : colonne `profiles.agency_role` + fonction `is_founder()` + RLS `invoices` réservé aux fondateurs (membre = tout sauf Finance & Accès). ⚠️ **APRÈS le SQL, redéployer `create-access`** (gate fondateur + enregistre `agency_role`). | [`2026-09-08-agency-roles.sql`](2026-09-08-agency-roles.sql) + `supabase functions deploy create-access --project-ref zizvggziggswhrbuyhuo` | ⏳ **À faire** | À vérifier : anon SELECT `invoices` = 200 `[]` ; après login membre, `invoices` = vide + Finance/Accès masqués ; création d'accès membre depuis un compte fondateur. |
