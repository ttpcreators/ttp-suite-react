...
| **Vivier créateurs** : table `creator_pool` (nom, handle, email, tag, note, last_contacted) — répertoire de créateurs hors roster à solliciter, RLS **agence-only** | [`creator-pool.sql`](creator-pool.sql) | ⏳ **À lancer** | — (après : `select=*` sur `creator_pool` = 200 `[]` en anon ; INSERT anon = 401) |
