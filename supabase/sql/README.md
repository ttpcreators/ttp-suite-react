...
| **Idées : sous-tâches** : colonne `ideas.subtasks` (jsonb) — checklist repliable sur chaque idée (agence + créateur, RLS `ideas` existante) | [`ideas-subtasks.sql`](ideas-subtasks.sql) | ⏳ **À lancer** | — (après : `select=subtasks` sur `ideas` doit renvoyer 200) |
