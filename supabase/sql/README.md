...
| **↳ Redéployer `create-access`** (réservé aux fondateurs + enregistre `agency_role` du nouveau compte) | `supabase functions deploy create-access --project-ref zizvggziggswhrbuyhuo` (⚠️ **Terminal**, pas SQL Editor) | ⏳ **À faire** | Recommandé mais non bloquant : sans lui, tout nouveau compte « Agence » est déjà créé en **member** (défaut colonne) ; le redéploiement verrouille la page Accès aux fondateurs + permet de nommer un nouveau fondateur. |
