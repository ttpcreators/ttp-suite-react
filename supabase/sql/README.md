...
| **Contacts : dernier contact** : colonne `contacts.last_contacted` (timestamptz) — anti sur-contact (MàJ auto à l'envoi + réconciliation Gmail) | [`contacts-last-contacted.sql`](contacts-last-contacted.sql) | ⏳ **À lancer** | — (après : `select=last_contacted` sur `contacts` doit renvoyer 200) |
| **Briefs : PDF joint** : colonne `briefs.pdf` (jsonb) — PDF uploadé + ligne `documents` (type brief) → visible Docs + portail créateur | [`briefs-pdf.sql`](briefs-pdf.sql) | ⏳ **À lancer** | — (après : `select=pdf` sur `briefs` doit renvoyer 200) |
