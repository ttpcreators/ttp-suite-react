...
| **Feuille de route partagée** : table `creator_roadmap` (agence écrit `roadmap` ↔ créateur lit + reporte sa cadence dans `self_cadence`) + trigger `creator_roadmap_guard` (le créateur ne peut pas modifier sa feuille de route) | [`creator-roadmap.sql`](creator-roadmap.sql) | ⏳ **À lancer** | — |
